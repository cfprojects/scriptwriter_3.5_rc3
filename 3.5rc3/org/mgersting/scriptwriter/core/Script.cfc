<cfcomponent displayname="Script" extends="org.mgersting.scriptwriter.core.ElementBase" output="no">
	...
	<cffunction name="init" access="public" returntype="org.mgersting.scriptwriter.core.ElementBase">
		...
		<cfscript>
			...
			setExternalOutputFormat("<script type=""text/javascript"" src=""#getPreferredPath()#""></script>");
			...
			if (arguments.src NEQ "") {
				if (getMinify()) {
					setInlineOutputFormat("<script type=""text/javascript"">#getSourceMin()#</script>");
				}
				else {
					setInlineOutputFormat("<script type=""text/javascript"">#arguments.src#</script>");
				}
			}
