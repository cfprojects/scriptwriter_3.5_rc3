<cfscript>
	...
	if (fb_.verbInfo.executionMode is "start")
	{
		...
		// We must have a contentvariable
		if (NOT StructKeyExists(fb_.verbInfo.attributes,"contentvariable")) {
			fb_throw(
				"fusebox.badGrammar.requiredAttributeMissing",
				"Required attribute is missing",
				"The attribute 'contentvariable' is required, for a 'scriptwriter' verb in fuseaction #fb_.verbInfo.circuit#.#fb_.verbInfo.fuseaction#."
			);
		}
		
		fb_.verbInfo.output = "<cfset #fb_.verbInfo.attributes.contentvariable# = #fb_.verbInfo.attributes.factory#.output(";
